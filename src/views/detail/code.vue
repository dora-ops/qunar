...
export default {
  ...
  methods: {
    getClassData() {
      ...
      var sql = classData.getClassData.replace("?", id);
      var p = this.$http.post("action", { sql: sql });

      sql = chapterlist.getChapterlist.replace("?", id);
      var p1 = this.$http.post("action", { sql: sql });
      sql = intro.getIntro.replace("?", id);
      var p2 = this.$http.post("action", { sql: sql });
      sql = teacherOpt.getTeacher.replace("?", id);
      var p3 = this.$http.post("action", { sql: sql });
      sql = rate.getrate.replace("?", id);
      var p4 = this.$http.post("action", { sql: sql });
      Promise.all([p, p1, p2, p3, p4]).then(([res, res1, res2, res3, res4]) => {
        var data = res.data[0];
        data.chapterlist = res1.data.map(item=>{
            item.classlist=JSON.parse(item.classlist)
            return item
        });
        data.intro = res2.data[0];
        data.teacher = res3.data[0];
        var rate = res4.data[0];
        rate.rate = JSON.parse(rate.rate);
        sql = commentlist.getcommentlist.replace("?", rate.id);
        this.$http.post("action", { sql: sql }).then(res => {
          rate.commentlist = res.data;
          data.rate = rate;
          var classData = { classData: data };
          //    debugger
          this.classData = data;
          this.intro = classData.classData.intro;
        });
      });

      // sql = chapterlist.getChapterlist.replace("?", id);
      // this.$http.post("action", { sql: sql }).then(res => {
      //   data.chapterlist = res.data[0];
      // });
      // sql = intro.getIntro.replace("?", id);
      // this.$http.post("action", { sql: sql }).then(res => {
      //   data.intro = res.data[0];
      // });
      // sql = teacherOpt.getTeacher.replace("?", id);

      // this.$http.post("action", { sql: sql }).then(res => {
      //   data.teacher = res.data[0];
      // });
      // sql = rate.getrate.replace("?", id);
      // this.$http.post("action", { sql: sql }).then(res => {
      //   var rate = res.data[0];
    ...
    addToCar(e) {
      if (this.$storage.state.userInfo) {
